...
#
# 【双击我没反应、或者 Mac 说要去「显示简介」改权限？】
# 那不是出错，是这个文件在传给你的路上把「可执行」标记弄丢了（下载都会）。
# 而「显示简介」那个面板改不了这个标记，照它说的做是白折腾。
# 解法是一行，打开终端粘这个：
#
#     bash ~/Downloads/装.command
#
# 开头那个 bash 是关键：双击是让系统「执行」这个文件，要标记；
# bash 文件名 是让 bash「读」这个文件照着做，读不需要标记。
